...
static void ExtractKtaPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    while (temp < 63.4f)
    {
        // Proteção: se temp for 0, evita loop infinito
        if (temp <= 0.0f)
        {
            // EEPROM claramente inválida (todos Kta = 0)
            // Mantém ktaScale1 = 0 e sai do loop
            break;
        }
